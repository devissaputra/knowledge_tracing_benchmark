...
def update_mastery(
    p_mastery: float, correct: bool, params: BKTParams | None = None
) -> float:
    """Apply one Bayesian Knowledge Tracing update."""
...
def trace(sequence, params: BKTParams | None = None) -> list[float]:
    """Return the mastery probability after each binary response."""
    params = params or BKTParams()
